...
function RabbitService( handler ) {
  ...
  if( 'function' !== typeof handler ) {
    handler = RabbitService.utility.noop;
  }
...
Object.defineProperties( RabbitService.prototype, {
  ...
  validate: {
    ...
    value: function validate() {
      ...
      var _parse = this.get( 'settings.url' ) ? RabbitService.utility.url_parse( this.get( 'settings.url' ) ) : {};

      // Convert URL to Keys if it is set and not the default
      if( this.get( 'settings.url' ) && this.get( 'settings.url' ) !== 'amqp://127.0.0.1:5672/' ) {
        this.set( 'settings.host', _parse.hostname );
        this.set( 'settings.port', _parse.port );
        this.set( 'settings.vhost', _parse.pathname || '/' );
        this.set( 'settings.login', _parse.auth ? _parse.auth.split( ':' )[0] : '' );
        this.set( 'settings.login', _parse.auth ? _parse.auth.split( ':' )[1] : '' );
      }
      ...
      this.set( 'environment', {
        RABBITMQ_NODE_IP_ADDRESS: this.get( 'settings.host', '127.0.0.1' ),
        RABBITMQ_NODE_PORT: this.get( 'settings.port', 5672 ),
