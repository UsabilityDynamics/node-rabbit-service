...
function RabbitService() {
  ...
  try {
    ...
    self.set( 'environment', {
      ...
      RABBITMQ_BASE: self.get( 'settings.directories.dynamic' ) + '',
      // RABBITMQ_CONFIG_FILE: self.get( 'settings.directories.config' ), // etc/rabbitmq/rabbitmq
      RABBITMQ_MNESIA_BASE: self.get( 'settings.directories.lib' ),
      RABBITMQ_MNESIA_DIR: self.get( 'settings.directories.cache' ),
      RABBITMQ_LOG_BASE: self.get( 'settings.directories.log' ), // /var/log/rabbitmq
      //RABBITMQ_SASL_LOGS: self.get( 'settings.directories.logs' ) + '',
      //RABBITMQ_PLUGINS_DIR: self.get( 'settings.directories.logs' ) + '/server-log.log',
      //RABBITMQ_PLUGINS_EXPAND_DIR: self.get( 'settings.directories.logs' ) + '/server-log.log',
      //RABBITMQ_ENABLED_PLUGINS_FILE: self.get( 'settings.directories.logs' ) + '/server-log.log',
      RABBITMQ_PID_FILE: self.get( 'settings.directories.pid' ) + '/rabbitmq'
    });

    //return console.log( require( 'util' ).inspect( self.get( 'environment' ), { showHidden: true, colors: true, depth: 2 } ) )

    // Validate Settings
    if( !self.get( 'validation.is_valid' ) ) {
      throw new Error( 'Validation Failure: ' + self.get( 'validation.errors' ) );
    }
    ...
    self.process = RabbitService.utility.spawn( self.get( 'bin.server' ), [ "__detached" ], {
      cwd: self.get( 'directories.dynamic' ),
    ...
    self.process.on( 'exit', function() {
      self.emit( 'server.exit', arguments );
