...
function RabbitService() {
  ...
  try {
    ...
    self.set( 'environment', {
      ...
      RABBITMQ_CONFIG_FILE: self.get( 'settings.directories.config' ) + '/rabbitmq', // etc/rabbitmq/rabbitmq
      RABBITMQ_MNESIA_BASE: self.get( 'settings.directories.dynamic' ) + '/mnesia',
      RABBITMQ_MNESIA_DIR: self.get( 'settings.directories.cache' ),
      RABBITMQ_LOG_BASE: self.get( 'settings.directories.logs' ), // /var/log/rabbitmq
